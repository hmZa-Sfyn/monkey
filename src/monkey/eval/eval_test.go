...
func TestFileObjects(t *testing.T) {
	tests := []struct {
		...
		{`let f = open("../parser/test_files/module.mk");str(f)`, "<file object: ../parser/test_files/module.mk>"},
		{`let f = open("../parser/test_files/module.mk");f.read()`, `import eval
import test
import sub_package`},
		{`let f = open("../parser/test_files/module.mk");f.readline()`, "import eval"},
		{`let f = open("../parser/test_files/module.mk");f.readline();f.readline()`, "import test"},
		{`let f = open("../parser/test_files/module.mk");f.readline();f.readline();f.readline()`, "import sub_package"},
	}
	d, _ := os.Getwd()
	fmt.Println(d)
