Below demonstrates some features of the Monkey language:
...
using (f = open("./file.log", "r")) {
    for line in <$f> where line =~ ``Monkey`` {
        println(line) //print only lines which match 'Monkey'
    }
}
```
